...
int highestProductOf3(const vector<int>& vectorOfInts)
{
    int highest   = max(vectorOfInts[0], vectorOfInts[1]);
    int lowest    = min(vectorOfInts[0], vectorOfInts[1]);
    int maxprod2  = vectorOfInts[0] * vectorOfInts[1];
    int minprod2  = vectorOfInts[0] * vectorOfInts[1];
    int maxprod3  = vectorOfInts[0] * vectorOfInts[1] * vectorOfInts[2];
    for (int i = 2; i < vectorOfInts.size();i++)
    {
        int current = vectorOfInts[i];
        maxprod3 = max(max(maxprod3, current*maxprod2) , current*minprod2);
        maxprod2 = max(max(maxprod2, current*highest)  , current*lowest);
        minprod2 = min(min(minprod2, current * lowest) , current*highest);
        highest = max(highest, current);
        lowest = min(lowest, current);
    }
    return maxprod3;
...
//int main(int argc, char** argv)
//{
//    std::cout << highestProductOf3({ {6, 1, 3, 5, 7, 8, 2} });
//    return 0;
//}